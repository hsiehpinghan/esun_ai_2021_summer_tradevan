...
class TestImageEntity():
    ...
    def _test_get_word_error_words(self) -> None:
        df = pd.read_csv(
            filepath_or_buffer='/home/hsiehpinghan/git/esun_ai_2021_summer_tradevan/data/模型訓練資料_e_o_corrected.csv')
        df = df[df['word'] == 'e']
    ...
    def _test_get_word_esun_800_words(self) -> None:
        df = pd.read_csv(filepath_or_buffer='/home/hsiehpinghan/git/esun_ai_2021_summer_tradevan/data/模型訓練資料_corrected.csv',
                         index_col='id')
    ...
    def _test_get_word_hadwritting_common_words_v2(self) -> None:
        df = pd.read_csv(
            filepath_or_buffer='/home/hsiehpinghan/git/esun_ai_2021_summer_tradevan/data/hadwritting_common_words_v2.csv')
        df = df[(df['word'] != 'e') & (df['word'] != 'o')]
        words = pd.unique(values=df['word'])
